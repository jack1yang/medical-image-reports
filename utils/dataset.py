    ...
    def __getitem__(self, index):
        ...
        for i, sentence in enumerate(text):
            ...
            if len(sentence) >= self.n_max-1:
                ...
                tokens.append(self.vocab('<end>'))
                for _ in range(self.n_max - len(tokens)):
                    tokens.append(self.vocab('<pad>'))
            # if max_word_num < len(tokens):
            #     max_word_num = len(tokens)
            target.append(tokens)
        for j in range(i+1, self.s_max): # pad to s_max sentences
            tokens = list()
            tokens.append(self.vocab('<start>'))
            tokens.append(self.vocab('<end>'))
            for _ in range(self.n_max - len(tokens)):
...
def collate_fn(data):
    ...
    images_frontal, images_lateral, images_name, captions = zip(*data)
    images_frontal = torch.stack(images_frontal, 0)
    images_lateral = torch.stack(images_lateral, 0)


    
    targets = np.zeros((len(captions), 8, 50))
    prob = np.zeros((len(captions), 8))
...
if __name__ == '__main__':
    ...
    image_dir = 'NLMCXR_png_pairs'
    caption_json = '.json'
    file_list = 'train_split.txt'
    batch_size = 6
    resize = 320
    crop_size = 224
